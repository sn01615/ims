...
$(document).ready(function(e){
        ...
        $.ajax({
            ...
            success:function(result,state){
                    ...
                    if(list && list.length >0){
                        $.each(list,function(index,it){
                            template+='<tr><td><input class="boxid" type="checkbox" name=""  data-id="'+it.user_id+'" /></td>'
                                +'<td>'+(serial+index+1)+'</td><td>'+it.username+'</td><td>'+it.realname+'</td><td>'+(it.telephone === null ? "" : it.telephone)+'</td>'
                                +'<td><a class="fontBtn permission_setting_u">'+lang.usermanage_biz.user_list.seting+'</a></td>'
                                +'<td><i class="icon-edit iconBtnH editUser" title="'+lang.usermanage_biz.user_list.edit+'"></i></td>'
                                +'<td><i class="icon-trash iconBtnH singleDelete" title="'+lang.usermanage_biz.user_list.cancel+'"></i></td></tr>';
                        });
                ...
                $(".singleDelete").on('click',function(){
                    ...
                    confirmFun(function(){
                        $.get('?r=api/deleteUser',{'uid':userId,'_rnd':loading()},function(result,state){
                            removeloading();
            ...
            if (status = 'success') {
                if (data.Ack == 'Success') {
                    ...
                    for (var x in data.Body.shops) {
                        ...
                        _html += '<label><input type="checkbox" value="' + data.Body.shops[x].shop_id + '"' + _sel + '>' + data.Body.shops[x].nick_name + '</label>';
                    }
    ...
    $("#set_user_shop_window_btn").on('click',function(e){
        ...
        $.post('?r=api/SetUserShops',{set:_set,userid:$("#set_user_shop_window_btn").data('userid')},function(data,status){
