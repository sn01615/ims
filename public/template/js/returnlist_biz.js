				...
				if(data.Ack ==='Success'){
					...
					if(M && M.length > 0){
						for(var i in M){
							...
							$('<tr id="'+M[i].return_request_id+'"><td><span>'+M[i].returnId_id+'<span></td><td><span>'+M[i].item_id
							+'</span></td><td><span>'+M[i].nick_name+'</span></td><td><span>'+cust+'</span></td><td><span>'+ucfirst(M[i].S_CI_reason)
							+'</span></td><td><span>'+intToLocalDate(M[i].creationDate,3)+'</span></td><td class="status"><span>'+ucfirst(M[i].S_state)+'</span></td><td><span>'+intToLocalDate(M[i].responseDue_respondByDate,3)
							+'</span></td><td title="'+lang.returnlist_biz.title+'"  class="returndetail"><span><a  class="fontBtn">'+lang.returnlist_biz.more_detail+'</a></span></td></tr>').appendTo('#ReturnList');
						}
