        ...
        $.get('?r=api/GetReturnDetail',global_info,function(data,status){
            if(status ==='success' && data.Ack ==='Success'){
                ...
                if(result.S_state !=='CLOSED'){
                    topShow = '<li><span class="leftName">'+lang.returndet_biz.top.deadline+'</span><span class="rightVal">'+intToLocalDate(result.S_sRD_respondByDate,3)+'</span></li>';
                } else {
                    topShow = '<li><span class="leftName">'+lang.returndet_biz.top.end_time+'</span><span class="rightVal">'+intToLocalDate(result.closeTime,3)+'</span></li>';
                }
                if(result.D_closeReason && result.S_state ==='CLOSED'){
                    topShow +='<li><span class="leftName">'+lang.returndet_biz.top.close_reason+'</span><span class="rightVal">'+ucfirst(result.D_closeReason)+'</span></li>';
                    
                }
                if(result.D_rSI_sT_trackingNumber){
                    topShow +='<li><span class="leftName">'+lang.returndet_biz.top.courier+'</span><span class="rightVal"> ' +result.D_rSI_sT_carrierUsed+'</span></li>'+
                            '<li><span class="leftName">'+lang.returndet_biz.top.track_num+'</span><span class="rightVal"> '+result.D_rSI_sT_trackingNumber+'</span></li>'+
                            '<li><span class="leftName">'+lang.returndet_biz.top.track_status+'</span><span class="rightVal"> '+ucfirst(result.D_rSI_sT_deliveryStatus)+'</span></li>';
                }
                if(result.S_eI_eBPCaseId){
                    topShow +='<li><span class="leftName">'+lang.returndet_biz.top.case_id+'</span><span class="rigthVal"> ' +result.S_eI_eBPCaseId + '</span></li>'+
                             '<li><span class="leftName">'+lang.returndet_biz.top.case_type+'</span><span class="rigthVal"> ' +result.S_eI_caseType + '</span></li>';
                }
                if(topShow ===''){
                    $('#top').hide();
                } else {
                ...
                //left部分
                $('<li><span class="leftName">'+lang.returndet_biz.left.account_id+'</span><span class="rightVal">'+result.S_sellerLoginName+'</span></li>'+
                  '<li><span class="leftName">'+lang.returndet_biz.left.return_id+'：</span><span class="rightVal">'+result.returnId_id+'</span></li>'+
                  '<li><span class="leftName">'+lang.returndet_biz.left.create_date+'</span><span class="rightVal">'+intToLocalDate(result.creationDate,3)+'</span></li>'+
                  '<li><span class="leftName">'+lang.returndet_biz.left.return_state+'</span><span class="rightVal">'+ucfirst(result.S_state)+'</span></li>'+
                  '<li><span class="leftName">'+lang.returndet_biz.left.return_reason+'</span><span class="rightVal" data-reason="'+result.S_CI_reason+'" id="requestreason">'+ucfirst(result.S_CI_reason)+'</span></li>').appendTo("#left");
                //right部分
                $('<li><span class="leftName">'+lang.returndet_biz.right.buyer_id+'</span><span class="rightVal"><a class="fontLinkBtn" href="http://www.ebay.com/usr/'+result.S_buyerLoginName+'" target="_blank">'+result.S_buyerLoginName+'</a></span></li>'+
                ...
                $('<table class="table">'+
                        ...
                        '<tbody><tr data-itemid="'+result.D_iD_itemId+'">'+
                            '<td><img src="" alt="" /></td>'+
                            '<td><span></span></td>'+
                            '<td><a href="http://cgi.ebay.com/ws/eBayISAPI.dll?ViewItem&item='+(_USER?result.D_iD_itemId.replace(/(\d{8})\d{4}/,'$1****'):result.D_iD_itemId)+'" target="_blank" class="fontLinkBtn">'+(_USER?result.D_iD_itemId.replace(/(\d{8})\d{4}/,'$1****'):result.D_iD_itemId)+'</a></td>'+
...
//                            }
                            ...
                            (function(){
                                ...
                                data.Body.regaddr_Phone.length<=0?'':RaddrStr+=lang.returndet_biz.register_info.telephone+data.Body.regaddr_Phone+', ';
                                
                                if(RaddrStr.length>12){
                                    $("#window_client_info_l").append('<li style=""><span class="leftName">'+lang.returndet_biz.register_info.register_addr+'</span><span class="rightVal" style="width:370px;">'
                                        +RaddrStr+'</span></li>');
                                }
                            })();
                                ...
                                if(data.Body.length>0){
                                    for(var x in data.Body){
                                        ...
                                        for(var y in data.Body[x].ExtTrans){
                                            transStr+=('<div class="textleft tooltip_td" title="'+lang.returndet_biz.buy_his.transaction_time+intToLocalDate(data.Body[x].ExtTrans[y].ExternalTransactionTime,3)
                                                +'<br />'+lang.returndet_biz.buy_his.payment+data.Body[x].ExtTrans[y].PaymentOrRefundAmount+data.Body[x].ExtTrans[y].PaymentOrRefundAmount_currencyID+''
                                                +'<br />'+lang.returndet_biz.buy_his.credit_amount+data.Body[x].ExtTrans[y].FeeOrCreditAmount+data.Body[x].ExtTrans[y].FeeOrCreditAmount_currencyID+''
                                                +'<br />'+lang.returndet_biz.buy_his.transaction_status+data.Body[x].ExtTrans[y].ExternalTransactionStatus+''
...
//                                        
                                        $("#buyHistory_div").find('tbody').append('<tr data-key='+x+'><td>'+data.Body[x].SellerUserID
                                                ...
                                                +data.Body[x].PaymentMethod+transStr+'</td>'+'<td><a href="javascript:;" title="'+addressStr+'" class="tooltip_td">'+lang.returndet_biz.buy_his.check+'</a></td>'
                                                ...
                                                if(data.Ack=='Success'){
                                                    ...
                                                    for(var x in data.Body){
                                                        noteText[x]=(lang.returndet_biz.buy_his.notes.author+data.Body[x].author_name
                                                            +'<br />'+lang.returndet_biz.buy_his.notes.content+data.Body[x].text
                                                            +'<br />'+lang.returndet_biz.buy_his.notes.cust+data.Body[x].cust
                                                            +'<br />'+lang.returndet_biz.buy_his.notes.create_time+intToLocalDate(data.Body[x].create_time,7));
                                                    }
                                                    ...
                                                    $("#buyHistory_div").find('tr[data-key='+_key+']').find('td[t="note"]').html(lang.returndet_biz.buy_his.notes.no_note);
                                                }else{
                                    ...
                                    $("#buyHistory_div").find('tbody').append('<tr><td colspan="10">'+lang.returndet_biz.buy_his.no_transaction+'</td></tr>');
                                }
                            }else{
                                // hintShow('hint_f','服务器错误!');
                            }
                                    ...
                                    if(data.Body.length>0){
                                        for(var x in data.Body){
                                            $his_msgs_ul.append('<li data-msgid="'+data.Body[x].msg_id+'" title="'+data.Body[x].Subject+'">'
                                                ...
                                                +((data.Body[x].FolderID==1)? lang.returndet_biz.msg_his.send:lang.returndet_biz.msg_his.receive)
    ...
    function getEbayOrderNote(returnid){
        $.get('?r=api/GetEbayOrderNote&returnid='+returnid,function(data,status){
            if(status =='success'){
                if(data.Ack =='Success'){
                    if(data.Body.Note){
                        $('.customerNote').html('<span class="leftName">'+lang.returndet_biz.order_note.cust_note+'</span><span>'+data.Body.Note+'</span>').show();
                    }
                    ...
                    if(addr){
                        ...
                        $('.custaddr').html('<span class="leftName">'+lang.returndet_biz.order_note.shipping_addr+'</span><span class="rightVal">'+shippingAddr+'</span>').show();
                        ...
                        var trackingNumber = addr.ShipmentTrackingNumber?'<a href="http://www.ec-firstclass.org/?track_number='
                            +addr.ShipmentTrackingNumber+'" target="_blank" class="fontLinkBtn">'+addr.ShipmentTrackingNumber+'</a>'+'<br />':'';
        ...
        $.get('?r=api/GetOredersRefund&returnid='+global_info.returnid,function(data,status){
            if(status ==='success' && data.Ack ==='Success'){
                ...
                for(var k in refund){
                    ...
                    var topRefund  ='';
                    if (ReferenceID){
                        topRefund += '<li><span class="leftName">'+lang.returndet_biz.order_refund.paypal_transaction_id+'</span><span class="rightVal">'+ReferenceID+'</span></li>';
                    }
                    if(RefundTime){
                        topRefund += '<li><span class="leftName">'+lang.returndet_biz.order_refund.refund_time+'</span><span class="rightVal">'+RefundTime+'</span></li>';
                    }
                    $('#top').show();
  ...
  */
    ...
    function getItemNoteList(itemId){
       ...
       setTimeout(function(){ $.ajax({
            ...
            data:{"itemId":itemId,"type":"return","returnid":global_info.returnid},
                    ...
                    if(M && M.length>0){
                        ...
                        for(var i in M){
                            ...
                            if(M[i].author == 'SELLER'){
                                $('<li class="sellerMSG">'+
                                '<div class="MSGC">'+
                                '<p><b>'+intToLocalDate(M[i].creationDate,3)+'</b>&nbsp;'+M[i].author+'<span class="note_md" data-note-md5='+M[i].note_md5+' data-activity-md5='+M[i].activityDetial_description_md5+'></span></p>'+
                                '<p>'+note+'</p>'+
                                '</div></li>').appendTo("#history");
                            }else{
                                $('<li class="buyerMSG">'+
                                '<div class="MSGC">'+
                                '<p><b>'+intToLocalDate(M[i].creationDate,3)+'</b>&nbsp;'+M[i].author+'<span data-note-md5='+M[i].note_md5+' data-activity-md5='+M[i].activityDetial_description_md5+'></span></p>'+
...
//                          option += '<option value="doc">'+lang.returndet_biz.return_deal.docs+'</option>';//@todo API 上传图片的还有些问题
    ...
    function getReturnAddr(returnid)
    {
        ...
        $.get('?r=api/GetReturnAddr&returnid='+returnid,function(data,status){
            if(status==='success' && data.Ack==='Success'){
                ...
                var returnAddr = result.name+' '+result.street1+' '+result.street2+' '
                +result.city+' '+result.county+' '+result.stateOrProvince+' '+ result.country+' '+result.postalCode+' '+result.any
    ...
    $('#determine').on('click',function(){
        ...
        for(var i in img){
            $('<li><i class="icon-close iconBtnS" title="'+lang.returdet_biz.upload_pic.del_pic+'"></i><img src="'+img[i]+'" width="70px" height="60px"></li>').appendTo('.imglist_addwindow');
        }
...
//            maxFiles:5,
            ...
            onUploadSuccess: function(id, data){
                if(data.Ack == 'Success'){
                    ...
                    $('<li><i class="icon-close iconBtnS" title="'+lang.returndet_biz.upload_pic.del_pic+'" data-id="'+id+'"></i><img src="'
    ...
    $('#addFile').on('click',function(){
        ...
        $('.imglist_addwindow').empty();
        for(var i in img){
            $('<li><i class="icon-close iconBtnS" title="'+lang.returndet_biz.upload_pic.del_pic+'"></i><img src="'+img[i]+'" width="70px" height="60px"></li>').appendTo('.imglist_addwindow');
        }
    });
    // 点击关闭添加文件窗口
