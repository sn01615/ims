...
$(document).ready(function(e) {
			...
			if (status == 'success' && data.Ack == 'Success') {
				...
				if (M && M.length > 0) {
					for (var i in M) {
						...
						$('<tr><td><input type="checkbox" class="ids" data-id="' + M[i].ebay_feedback_transaction_id + '" data-order-date="' + M[i].CreatedDate +
							'"/></td><td class="shop"><span>' + M[i].nick_name + '</span></td><td class="Client"><span>' + M[i].CommentingUser +
							'</span></td><td class="itemInfo" data-itemid="' + M[i].ItemID + '"><span>' + M[i].ItemID +
							'</span></td><td class="evaluate" data-type="' + M[i].CommentType + '"><i class="' + type + '"></i><span>' + M[i].CommentText +
							'</span></td><td class="CommentTime"><span>' + intToLocalDate(M[i].CommentTime, 3) + '</span></td><td><span>' +
							lastSendTime + '</span></td><td title="' + lang.feedbacklist_biz.click_view_details +
							'"  class="feedbackDet" data-orderLineItemID="' + M[i].OrderLineItemID + '" data-isSendMsg="' + M[i].isSendMsg +
							...
							'" data-feedbackOutDate="' + M[i].isFeedbackOuteDate + '" data-declineChange="' + M[i].isDeclineChange +
							'"><span><a class="fontBtn">' + lang.feedbacklist_biz.details + '</a></span></td></tr>').appendTo('#feedbackList');
					}
				}
	...
	(function() {
		$('#feedbackList').on('click', '.feedbackDet', function() {
			...
			if (!!responseText || isResponse == 1) {
				...
				$('#feedback_length').after('<small>' + lang.feedbacklist_biz.cannot_reply + '</small><p>' + lang.feedbacklist_biz.reply_content +
					'<span>' + responseText + '</span></p>');
			}
			var type;
			global_item = itemid;
			...
			(function() {
				...
				$('<tr><td>' + shopId +
					'</td><td class="picture"></td><td id="itemVarition"></td><td id="itemQuan"></td><td id="orderCreateTime" data-order-time=""></td><td id="itemPrice"></td></tr>'
				).appendTo('#orderList');
				...
				$.get('?r=api/GetItemURL', {
					'itemid': itemid
				}, function(data, status) {
					if (status == 'success' && data.Ack == 'Success') {
						var gallery_url = data.Body.gallery_url;
						$('.picture').html('<a href="' + gallery_url + '" data-lightbox="example-set-12"><img alt="" src="' +
							gallery_url.replace(/\_1\.JPG\?/i, '_0.JPG?') + '"></img></a>');
					}
				})
			})();
			...
			$.get('?r=api/GetFeedbackOrder', {
				...
				if (status === 'success' && data.Ack === 'Success') {
					...
					price += (_obj.ActualShippingCost || _obj.ActualShippingCost > 0) ? _obj.ActualShippingCost + '&nbsp' + _obj.ActualShippingCost_currencyID +
						'<br />' : '';
					...
					if (status === 'success') {
						if (data.Ack === 'Success') {
							...
							(function() {
								...
								data.Body.regaddr_Phone.length <= 0 ? '' : RaddrStr += '<br />' + lang.feedbacklist_biz.phone +
									': ' + data.Body.regaddr_Phone + ', ';

								if (RaddrStr.length > 12) {
									$("#window_client_info_l").append('<li style=""><span class="leftName">' + lang.feedbacklist_biz
										.reg_address + '：</span><span class="rightVal" style="width:370px;">' + RaddrStr +
										'</span></li>');
								}
							})();
							...
							if (data.Body.length > 0) {
								for (var x in data.Body) {
									...
									for (var y in data.Body[x].ExtTrans) {
										transStr += ('<div class="textleft tooltip_td" title="' + lang.feedbacklist_biz.transaction_time +
											'：' + intToLocalDate(data.Body[x].ExtTrans[y].ExternalTransactionTime, 3) + '<br />' +
											lang.feedbacklist_biz.amount + data.Body[x].ExtTrans[y].PaymentOrRefundAmount + data.Body[
												x].ExtTrans[y].PaymentOrRefundAmount_currencyID + '' + '<br />' + lang.feedbacklist_biz
											.FVF + data.Body[x].ExtTrans[y].FeeOrCreditAmount + data.Body[x].ExtTrans[y].FeeOrCreditAmount_currencyID +
											'' + '<br />' + lang.feedbacklist_biz.transaction_status + data.Body[x].ExtTrans[y].ExternalTransactionStatus +
											'' + '">' + data.Body[x].ExtTrans[y].ExternalTransactionID + '(' + data.Body[x].ExtTrans[
												y].PaymentOrRefundAmount + data.Body[x].ExtTrans[y].PaymentOrRefundAmount_currencyID +
											')' + '</div>');
									}
									//                                     // 属性
									var _shuxing = '';
									...
									$("#buyHistory_div").find('tbody').append('<tr data-key=' + x + '><td>' + data.Body[x].SellerUserID +
										...
										'" class="tooltip_td">' + lang.feedbacklist_biz.view + '</a></td>' + '<td>' + orderStatus +
										'</td>' + '<td class="evaluate">' + _feedback + '</td>' +
											...
											if (data.Ack == 'Success') {
												...
												for (var x in data.Body) {
													noteText[x] = (lang.feedbacklist_biz.Notes.author + '：' + data.Body[x].author_name +
														'<br />' + lang.feedbacklist_biz.Notes.content + '：' + data.Body[
															x].text + '<br />' + lang.feedbacklist_biz.Notes.buyer +
														'：' + data.Body[x].cust + '<br />' + lang.feedbacklist_biz.Notes
												...
												$("#buyHistory_div").find('tr[data-key=' + _key + ']').find('td[t="note"]')
													.html(lang.feedbacklist_biz.none);
											} else {
												$("#buyHistory_div").find('tr[data-key=' + _key + ']').find('td[t="note"]')
													.html('✖');
											}
								...
								$("#buyHistory_div").find('tbody').append('<tr><td colspan="10">' + lang.feedbacklist_biz.not_data +
									'</td></tr>');
							}
						} else {}
					} else {}
									...
									for (var x in data.Body) {
										$his_msgs_ul.append('<li data-msgid="' + data.Body[x].msg_id + '" title="' + data.Body[x].Subject +
											'">' + '<span>' + intToLocalDate(data.Body[x].ReceiveDate, 9) + '</span><p>' + data.Body[
												x].nick_name + ' | ' + ((data.Body[x].FolderID == 1) ? lang.feedbacklist_biz.send +
												'：' : lang.feedbacklist_biz.receive + '：') + data.Body[x].Subject + '</p></li>');
									}
									// noloading = true;
			...
			(function() {
				function getSubClassHtml(pid, data) {
							...
							if (state === 'success') {
								if (listdata.Body.list !== '') {
									$.each(listdata.Body.list, function(index, item) {
										$("#temp_msg select[pid=" + pid + "]").append('<option value="' + item.tp_list_id +
											'">' + item.title + '</option>');
									});
					...
					$("#temp_msg").append('<select pid="' + pid + '"><option value="">' + lang.feedbacklist_biz.please_select +
						'</option></select>');
					for (var i in obj) {
						$("#temp_msg select[pid=" + pid + "]").append('<option value="' + obj[i].tp_class_id + '">' + obj[i].classname +
							'</option>');
					}
					$("#temp_msg select[pid=" + pid + "]").change(function(e) {
			...
			(function() {
				function getSubClassHtml(pid, data) {
					...
					if (obj.length == 0) {
						$.get("?r=api/GetTpList", {
							...
							if (state === 'success') {
								if (listdata.Body.list !== '') {
									$.each(listdata.Body.list, function(index, item) {
										$("#temp_feedback select[pid=" + pid + "]").append('<option value="' + item.tp_list_id +
											'">' + item.title + '</option>');
									});
					...
					$("#temp_feedback").append('<select pid="' + pid + '"><option value="">' + lang.feedbacklist_biz.please_select +
						'</option></select>');
					for (var i in obj) {
						$("#temp_feedback select[pid=" + pid + "]").append('<option value="' + obj[i].tp_class_id + '">' + obj[i].classname +
							'</option>');
					}
					$("#temp_feedback select[pid=" + pid + "]").change(function(e) {
						var npid = $(this).find('option:selected').val();
				...
				if (textStatus == 'success') {
					if (data.Ack === 'Success') {
						...
						$msgtext.find('img').each(function(index, element) {
							if ($(element).attr('src') !== undefined && $(element).attr('src').indexOf('button') == -1 && $(element).attr(
									'src').indexOf('ebaystatic') == -1) {
						...
						$msgtext.find('img').each(function(index, element) {
							if ($(element).attr('src') !== undefined && $(element).attr('src').indexOf('button') == -1 && $(element).attr(
									'src').indexOf('ebaystatic') == -1 && $(element).attr('src').indexOf('/globalAssets/') == -1 && $(
									element).attr('src').indexOf('/icon/') == -1 && $(element).attr('src').indexOf('/roveropen/') == -1) {
	...
	$('#determine').on('click', function() {
		...
		$('.imglist_addwindow').empty();
		for (var i in img) {
			$('<li><i class="icon-close iconBtnS" title="' + lang.feedbacklist_biz.click_delete + '"></i><img src="' + img[i] +
		...
		$("#drop-area-div").dmUploader({
			...
			onUploadSuccess: function(id, data) {
				if (data.Ack == 'Success') {
					...
					$('<li><i class="icon-close iconBtnS" title="' + lang.feedbacklist_biz.click_delete + '" data-id="' + id +
	...
	$('#sendMsg').on('click', function() {
		...
		loading();
		$.post('?r=Api/GenerateContactMsgQueue', {
	...
	(function() {
		...
		$bSendContent.on('input change', function() {
			...
			if (length < 0) {
				...
				$('#batch_msg_length').find('small').html(lang.feedbacklist_biz.input_number_l + '<span style="color: #f00;">' + length + '</span>' +
					lang.feedbacklist_biz.input_number_r);
			} else {
				$("#saveMsg").attr("disabled", false);
				$('#batch_msg_length').find('small').html(lang.feedbacklist_biz.input_number_l + '<span>' + length + '</span>' + lang.feedbacklist_biz
					.input_number_r);
			}
		});
	})();
	...
	(function() {
		...
		$oSendContent.on('input change', function() {
			...
			if (length < 0) {
				...
				hintShow('hint_w', lang.feedbacklist_biz.chars_number_error);
				$('#msg_length').find('span').html(lang.feedbacklist_biz.input_number_l + '<b style="color: #f00;">' + length + '</b>' + lang.feedbacklist_biz
					.input_number_r);
			} else {
	...
	(function() {
		...
		$oSendFeedback.on('input change', function() {
			...
			if (length < 0) {
				...
				hintShow('hint_w', lang.feedbacklist_biz.chars_number_error);
				$('#feedback_length').find('span').html(lang.feedbacklist_biz.input_number_l + '<b style="color: #f00;">' + length + '</b>' + lang.feedbacklist_biz
					.input_number_r);
			} else {
	...
	(function() {
		...
		$oSendBatchFeedback.on('input change', function() {
			...
			if (length < 0) {
				...
				hintShow('hint_w', lang.feedbacklist_biz.chars_number_error);
				$('#batch_length').find('small').html(lang.feedbacklist_biz.input_number_l + '<span style="color: #f00;">' + length + '</span>' +
					lang.feedbacklist_biz.input_number_r);
			} else {
	...
	(function() {
		$('#saveStatus').on('click', function() {
			...
			var feedbackStatus = {
				...
				'reqtime': RequestOutDate,
				'fedtime': FeedbackOutDate,
				'dechange': DeclineChange
			};
			...
			if (obj.length == 0) {
				$.get("?r=api/GetTpList", {
					...
					if (state === 'success') {
						if (listdata.Body.list !== '') {
							$.each(listdata.Body.list, function(index, item) {
								$("#temp_batch select[pid=" + pid + "]").append('<option value="' + item.tp_list_id + '">' + item.title +
									'</option>');
							});
