...
//                      '<li><span>消费：</span><span>暂时没数据</span></li>'+
                    ...
                    $('<table class="table">'+
                        ...
                        '<tbody><tr data-itemid="'+result.i_itemId+'">'+
                            '<td><img src="" alt="" /></td>'+
                            '<td><span></span></td>'+
                            '<td id="i_itemId"><span><a href="http://cgi.ebay.com/ws/eBayISAPI.dll?ViewItem&item='+(_user_id?result.i_itemId.replace(/(\d{8})\d{4}/,'$1****'):result.i_itemId)+'" target="_blank" class="fontLinkBtn">'
                                +(_user_id?result.i_itemId.replace(/(\d{8})\d{4}/,'$1****'):result.i_itemId)+'</a></span></td>'+
...
//                    });
                    ...
                    // 由itemid获取item信息
                    setTimeout(function(){getItemInfo(global_info.caseid,result.i_itemId)},1000);
                    //获取评价信息
                                ...
                                setTimeout(function(){
                                    ...
                                    $.get('?r=api/GetCountryList',function(data,status){
                                        if(status ==="success"){
                                            ...
                                            ///退货地址只显示在描述不符部分
                                            returnAddr = '<p class="returnAddr"> <span class="spanMode span100">'+lang.disputeDet_biz.address.country+'：</span>'+countryOptions+'</p>'
                                            +'<p class="returnAddr" id="state"> <span class="spanMode span100">'+lang.disputeDet_biz.address.state+'：</span><input type="text" name=""  /></p>'
                                            +'<p class="returnAddr" id="city"> <span class="spanMode span100">'+lang.disputeDet_biz.address.city+'：</span><input type="text" name="" id="" /></p>'
                                            +'<p class="returnAddr" id="street"> <span class="spanMode span100">'+lang.disputeDet_biz.address.street1+'：</span><input type="text" name="" id="" /></p>'
                                            +'<p class="returnAddr" id="street2"> <span class="spanMode span100">'+lang.disputeDet_biz.address.street2+'：</span><input type="text" name="" id="" /></p>'
                                            +'<p class="returnAddr" id="postcode"> <span class="spanMode span100">'+lang.disputeDet_biz.address.postcode+'：</span><input type="text" name="" id="" /></p>'
                                            +'<p class="returnAddr" id="name"> <span class="spanMode span100">'+lang.disputeDet_biz.address.contacts+'：</span><input type="text" name="" id="" /></p>'
                                            +'<p class="returnAddr" id="code"> <span class="spanMode span100">'+lang.disputeDet_biz.address.RMA+'：</span><input type="text" name="" id="" /></p>';
                    ...
                    $('#saveAddr').on('click',function(){
                        ...
                        if(  returnState == lang.disputeDet_biz.input_info.state || returnState == '' || returnCity == lang.disputeDet_biz.input_info.city || returnCity =='' || returnStreet == lang.disputeDet_biz.input_info.city || returnStreet =='' ||
                            ...
                            var Addr = returnName+' '+returnStreet+' '+returnStreet2+' '
                            +returnCity+' '+returnState+' '+ returnCountry+' '+returnZip
                        ...
                        var TrackingInfo = [];
                            switch (options){
                                case 'msg':
                                    ...
                                    param={'text':fileText,'type':result.caseId_type,'case_id':global.caseid,'caseId_id':result.caseId_id};
                                ...
                                case 'trackingInfo':
                                    ...
                                    param={'carrier':carrier,'num':trackingNum,'text':fileText,'type':result.caseId_type,'case_id':global.caseid,'caseId_id':result.caseId_id};
                                ...
                                case 'shippingInfo':
                                     ...
                                     param = {'carrier':carrier,'shipdate':get_unix_time(shipdate),'text':fileText,'type':result.caseId_type,'case_id':global.caseid,'caseId_id':result.caseId_id};
                                ...
                                case 'refund':
                                case 'fullRefund':
                                    param={'type':result.caseId_type,'case_id':global.caseid,'text':fileText,'caseId_id':result.caseId_id}
                                ...
                                case 'partRefund':
                                    ...
                                    param = {'type':result.caseId_type,'case_id':global.caseid,'amount':amount,'text':fileText,'caseId_id':result.caseId_id};
                                ...
                                case 'returnRefund':
                                    ...
                                    param = {'type':result.caseId_type,'case_id':global.caseid,'text':fileText,'caseId_id':result.caseId_id,'country':country,'state':state,'city':city,'street':street,'street2':street2,'name':name,'postcode':postcode,'code':code};
                                ...
                                case 'ebayHelp':
                                    ...
                                    param={'type':result.caseId_type,'case_id':global.caseid,'text':fileText,'reason':cause,'caseId_id':result.caseId_id};
                                ...
                                case 'provideReturnInfo':
                                    ...
                                    param ={'type':result.caseId_type,'case_id':global.caseid,'name':returnName,'street1':returnStreet,'street2':returnStreet2,
                                            'city':returnCity,'state':returnState,'country':returnCountry,'ZIP':returnZip,'RMA':returnRMA,'caseId_id':result.caseId_id};
                                ...
                                case 'appealEbay':
                                    ...
                                    param ={'type':result.caseId_type,'case_id':global.caseid,'appeal':appealVal,'text':fileText,'caseId_id':result.caseId_id};
                    ...
                    (function(){
                        ...
                        cashs.blur(function(){
                        	...
                        	if(isNaN(cost)){
                        		$('#cash small').empty();
                            	$('<small>'+lang.disputeDet_biz.input_amount2+'</small>').appendTo('#cash').css({'color':'red'});
                        	} else {
        ...
        function columnsBlur(obj,strText){
            obj.blur(function(){
                if(!obj.val() || typeof(obj.val()) === "undefined"){
                    obj.addClass('redBorder').val(strText).css({'color':'red','font-size':'12px'});
                } else {
                    obj.removeClass('redBorder').css({'color':'#666'});
                }
            });
            obj.focus(function(){
                if(obj.val() === strText){
...
//                      '<li><span>消费：</span><span>暂时没数据</span></li>'+
                    ...
                    $('<table class="table">'+
                        ...
                        '<tbody><tr data-itemid="'+result.i_ItemID+'">'+
                            '<td><img src="" alt="" /></td>'+
                            '<td><span></span></td>'+
                            '<td><a href="http://cgi.ebay.com/ws/eBayISAPI.dll?ViewItem&item='+(_user_id?result.i_ItemID.replace(/(\d{8})\d{4}/,'$1****'):result.i_ItemID)+'" target="_blank" class="fontLinkBtn">'
                                +(_user_id?result.i_ItemID.replace(/(\d{8})\d{4}/,'$1****'):result.i_ItemID)+'</a></td>'+
...
//                  getReturnHistory(returnId_id);
        ...
        function getCaseHandleLog(){
            var param = {'case_id':global_info.caseid,'_rnd':loading()}
                        ...
                        if($(element).html().toString().indexOf('Seller provided tracking information for shipment.')>-1){
                            for (var k in M){
                                if(M[k].handle_type ==='addTrackingInfo'){
                                  $(element).after('<br/><i>trackingNumber: <a href="http://www.ec-firstclass.org/?track_number='
                                   +M[k].trackingNum+'" target="_blank" class="fontLinkBtn">'
                                   +M[k].trackingNum+'</a></i>');
        ...
        function getItemNoteList(itemId){
           ...
           setTimeout(function(){ $.ajax({
                ...
                data:{"itemId":itemId,"type":"case","caseId":caseId},
        ...
        $('.dealTable thead .more').on('click',function(){
            ...
            if($this.attr('onBtn')=='off'){
                $('.dealTable tbody tr').show('0',function(){$this.attr('onBtn','on');});
                $this.html(lang.disputeDet_biz.UI.hide).attr('title',lang.disputeDet_biz.UI.click_hide);
            }else if($this.attr('onBtn')=='on'){
                $('.dealTable tbody tr:first').siblings().hide('0',function(){$this.attr('onBtn','off');});
                ...
                if(data.Ack==='Success'){
                    ...
                    (function(){
                        ...
                        if(RaddrStr.length>12){
                            $("#window_client_info_l").append('<li style=""><span class="leftName">'+lang.disputeDet_biz.reg_address+'：</span><span class="rightVal" style="width:370px;">'
                                +RaddrStr+'</span></li>');
                        }
                    })();
                    (function(){
                        ...
                        return 
                        // 其他地址显示
                        $.get('?r=api/GetUserAddress', {EIASToken:data.Body.EIASToken}, function(data,textStatus){
                            if(textStatus=='success'){
                                if(data.Ack=='Success'){
                                    for(var x in data.Body){
                                        ...
                                        $("#window_client_info_l").append('<li style=""><span class="leftName">'+lang.disputeDet_biz.shipping_address+(+x+1)+'：</span><span class="rightVal" style="width:370px;">'
                                            +addrStr+'</span></li>');
                                    }
                        ...
                        if(data.Body.length>0){
                            for(var x in data.Body){
                                ...
                                for(var y in data.Body[x].ExtTrans){
                                    transStr+=('<div class="textleft tooltip_td" title="'+lang.disputeDet_biz.transaction_info.time+'：'+intToLocalDate(data.Body[x].ExtTrans[y].ExternalTransactionTime,5)
                                        +'<br />'+lang.disputeDet_biz.transaction_info.amount+'：'+data.Body[x].ExtTrans[y].PaymentOrRefundAmount+data.Body[x].ExtTrans[y].PaymentOrRefundAmount_currencyID+''
                                        +'<br />'+lang.disputeDet_biz.transaction_info.FVF+'：'+data.Body[x].ExtTrans[y].FeeOrCreditAmount+data.Body[x].ExtTrans[y].FeeOrCreditAmount_currencyID+''
                                        +'<br />'+lang.disputeDet_biz.transaction_info.status+'：'+data.Body[x].ExtTrans[y].ExternalTransactionStatus+''
                                ...
                                $("#buyHistory_div").find('tbody').append('<tr data-key='+x+'><td>'+data.Body[x].SellerUserID
                                        ...
                                        +data.Body[x].PaymentMethod+transStr+'</td>'+'<td><a href="javascript:;" title="'+addressStr+'" class="tooltip_td">'+lang.disputeDet_biz.view+'</a></td>'
                                ...
                                (function(){
                                    ...
                                    $.get('?r=api/GetItemNotes',{itemId:data.Body[x].Item_ItemID},function(data,textStatus){
                                        if(data.Ack=='Success'){
                                            var noteText=[];
                                            for(var x in data.Body){
                                                noteText[x]=(lang.disputeDet_biz.Notes.author+'：'+data.Body[x].author_name
                                                    +'<br />'+lang.disputeDet_biz.Notes.content+'：'+data.Body[x].text
                                                    +'<br />'+lang.disputeDet_biz.Notes.buyer+'：'+data.Body[x].cust
                                                    +'<br />'+lang.disputeDet_biz.Notes.time+'：'+intToLocalDate(data.Body[x].create_time,7))+'<br /><br />';
                                            }
                            ...
                            if(data.Body.length>0){
                                for(var x in data.Body){
                                    $his_msgs_ul.append('<li data-msgid="'+data.Body[x].msg_id+'" title="'+data.Body[x].Subject+'">'
                                        ...
                                        +((data.Body[x].FolderID==1)?lang.disputeDet_biz.msg.send+'：':lang.disputeDet_biz.msg.receive+'：')
