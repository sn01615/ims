...
class EMongoDbHttpSession extends CHttpSession
{
   ...
   * @modify 2015-10-08 YangLong 升级与抛出异常
   */
  public function init()
  {
        try {
            $connection = new MongoClient('mongodb://' . $this->connectionString);
        } catch (Exception $e) {
            throw new CException(Yii::t('yii', 'oh, mongodb connect error, connectionString: {path}', array(
                '{path}' => 'mongodb://' . $this->connectionString
            )));
        }
    $dbName = $this->dbName;
