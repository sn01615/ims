...
class EbayUserModel extends BaseModel
{
    ...
    public function getEbayUserList($page, $pageSize, $sellerId, $keyword, $searchType)
    {
        ...
        $result['list'] = EbayUserInfoDAO::getInstance()->iselect($columns, $conditions, $params, true, $joinArray, 'u', $order, $limit, $offset, 
            'DISTINCT SQL_CALC_FOUND_ROWS', $groups);
        ...
        if ($result['list'] !== false) {
            foreach ($result['list'] as $key => &$value) {
                $value['_count_orders'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT count(*) FROM
                    ebay_orders as o
                    where o.EIASToken='{$value['EIASToken']}'", array(), 'queryScalar');
                $value['_count_trans'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT count(*) FROM
                    ebay_orders as o
                    join ebay_order_transaction as t
                    on o.ebay_orders_id=t.ebay_orders_id
                    where o.EIASToken='{$value['EIASToken']}'", array(), 'queryScalar');
                $value['_count_cases'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT caseId_type,count(*) as num
                    FROM `case` where otherParty_userId='{$value['UserID']}' or user_userId ='{$value['UserID']}' group by caseId_type", 
                    array(), 'queryAll');
                $value['_count_disputes'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT DisputeReason,count(*) as num
                    FROM `disputes` where BuyerUserID='{$value['UserID']}' group by DisputeReason", array(), 'queryAll');
                $value['_count_Neutral'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT count(*) FROM
                    ebay_feedback_transaction where CommentType='Neutral'
                    and CommentingUser='{$value['UserID']}'", array(), 'queryScalar');
                $value['_count_Negative'] = EbayUserInfoDAO::getInstance()->setTextQuery(
                    "SELECT count(*) FROM
                ...
                $value['usernames'] = EbayOrdersDAO::getInstance()->iselect($columns, $conditions, $params, true, $joinArray, 'o', '', 0, null, 
                    'DISTINCT');
            }
            unset($value);
