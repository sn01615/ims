...
class DisputesModel extends BaseModel
{
    ...
    public function executeDisputesDownQueue($parameters)
    {
        ...
        while (true) {
            $page ++;
            $xmllist = CaseDownModel::model()->getUserDisputes($parameters['start_time'], $parameters['end_time'], $parameters['token'], $parameters['site_id'], $page);
            ...
            if (stripos($xmllist, '<TotalNumberOfPages>' . $page . '</TotalNumberOfPages>') !== false || stripos($xmllist, '<TotalNumberOfPages>0</TotalNumberOfPages>') !== false) {
                break;
            }
        }
        return true;
                    ...
                    if (isset($value['data_xml']) ) {
                        ...
                        if (pq('Ack')->html()==='Success') {
                            ...
                            for ($i=0 ; $i<$dispute_length ; $i++){
                                ...
                                ->html();
                                $column_list = array(
                                    'DisputeCreatedTime' => strtotime($dispute->eq($i)->find('DisputeCreatedTime')->html()),
                                    'shop_id'=>$value['shop_id'],
                                    'DisputeCreditEligibility' => $dispute->eq($i)->find('DisputeCreditEligibility')->html(),
                                    'DisputeExplanation' => $dispute->eq($i)->find('DisputeExplanation')->html(),
                                    'DisputeID' => $disputeId,
                                    'DisputeModifiedTime' => strtotime($dispute->eq($i)->find('DisputeModifiedTime')->html()),
                                    'DisputeReason' => $dispute->eq($i)->find('DisputeReason')->html(),
                                    'DisputeRecordType' => $dispute->eq($i)->find('DisputeRecordType')->html(),
                                    'DisputeState' => $dispute->eq($i)->find('DisputeState')->html(),
                                    'DisputeStatus' => $dispute->eq($i)->find('DisputeStatus')->html(),
                                    'OrderLineItemID'=>$dispute->eq($i)->find('OrderLineItemID')->html(),
                                    'TransactionID'=>$dispute->eq($i)->find('TransactionID')->html(),
                                    'OrderLineItemID' => $dispute->eq($i)->find('OrderLineItemID')->html(),
                                    'OtherPartyName' => $dispute->eq($i)->find('OtherPartyName')->html(),
                                    'OtherPartyRole' => $dispute->eq($i)->find('OtherPartyRole')->html(),
                                    'TransactionID' => $dispute->eq($i)->find('TransactionID')->html(),
                                    'UserRole' => $dispute->eq($i)->find('UserRole')->html(),
                                    'create_time' => time()
                                );
                               
                                $_conditions = array('DisputeID'=>$column_list['DisputeID']);
                                ...
                                for($j=0; $j<$resolution_len; $j++) {
                                    $colum_resolution = array(
                                        'disputes_id' => $disputes_id,
                                        'DisputeResolutionReason' => $resolution->eq($j)->find('DisputeResolutionReason')->html(),
                                        'DisputeResolutionRecordType' => $resolution->eq($j)->find('DisputeResolutionRecordType')->html(),
                                        'ResolutionTime' => strtotime($resolution->eq($j)->find('ResolutionTime')->html())
                                    );
                                ...
                                for($k=0; $k<$msg_length; $k++) {
                                    $column_msg = array(
                                        'disputes_id' => $disputes_id,
                                        'MessageCreationTime' => strtotime($msg->eq($k)->find('MessageCreationTime')->html()),
                                        'MessageID' => $msg->eq($k)->find('MessageID')->html(),
                                        'MessageSource' => $msg->eq($k)->find('MessageSource')->html(),
                                        'MessageText' => $msg->eq($k)->find('MessageText')->html()
                                    );
                                    DisputeMsgDAO::getInstance()->iinsert($column_msg);
                                }
                                $detail = DisputesDownDAO::getInstance()->findByAttributes(
                                            array('type_flag'=>$disputeId),
                                            array('data_xml,disputes_down_id'),
                                            array('disputes_down_id ASC')
                                        );
                                if(!empty($detail)){
                                    $conditions_det = 'DisputeID =:dispute';
                                    $params_det = array(':dispute'=>$disputeId);
                                    ...
                                    try{
                                        if($ddoc['GetDisputeResponse>Ack']->html() =='Success'){
                                            ...
                                            $column_det = array(
                                                ...
                                                'i_ss_ConvertedCurrentPrice_currencyID'=>$det->find('Item>SellingStatus>ConvertedCurrentPrice')->attr('currencyID'),
                                        ...
                                        DisputesDAO::getInstance()->commit();
                                         ...
                                         iMongo::getInstance()->setCollection('DisputesDetailParseErr')->insert(array(
                                            'getCode' => $e->getCode(),
                                            'getFile' => $e->getFile(),
                                            'getLine' => $e->getLine(),
                                    ...
                                    iMongo::getInstance()->setCollection('MissingDisputesDetail')->insert(array(
                                    'DisputeID' => $disputeId,
                                    'getCode' => $e->getCode(),
                                    'getFile' => $e->getFile(),
                                    'getLine' => $e->getLine(),
